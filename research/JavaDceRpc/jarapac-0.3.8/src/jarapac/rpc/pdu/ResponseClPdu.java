...
public class ResponseClPdu extends ConnectionlessPdu implements Fragmentable {
    ...
    public Iterator fragment(int size) {
        byte[] stub = getStub();
        if (stub == null) {
            return Arrays.asList(new ResponseClPdu[] { this }).iterator();
        }
    ...
    public Fragmentable assemble(Iterator fragments) throws IOException {
        ...
        try {
            ResponseClPdu pdu = (ResponseClPdu) fragments.next();
            byte[] stub = pdu.getStub();
            if (stub == null) stub = new byte[0];
            while (fragments.hasNext()) {
                ResponseClPdu fragment = (ResponseClPdu) fragments.next();
    ...
    private class FragmentIterator implements Iterator {
        ...
        public Object next() {
            ...
            ResponseClPdu fragment = (ResponseClPdu) ResponseClPdu.this.clone();
